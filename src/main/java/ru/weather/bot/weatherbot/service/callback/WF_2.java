...
@Component
public class WF_2 implements CallBackHandler
{
    private final Deque<BotApiMethod<?>> stackMessages;
    private final WeatherMapper weatherMapper;

    public WF_2(Deque<BotApiMethod<?>> stackMessages, WeatherMapper weatherMapper)
    {
    ...
    public BotApiMethod<?> processCallBack(Update update)
    {
        ...
        weatherMapper.getReceiveData().getWeatherConfig().setCurrentDay(14);
        return EditMessageText.builder()
                .chatId(update.getMessage().getChatId())
                .messageId(update.getMessage().getMessageId())
                .text(weatherMapper.weatherForecastDay(cityName, 2 * 8 - 2))
